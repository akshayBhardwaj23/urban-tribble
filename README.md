...
**If checkout shows “Hosted page is not available”:** In Razorpay Dashboard ensure **Subscriptions** (and hosted subscription checkout, if prompted) are enabled for the account, and that you use **Plan IDs** that belong to the same mode as your API keys (test vs live). The app rewrites `api.razorpay.com` subscription links to `checkout.razorpay.com` where needed; if the error persists, open a ticket with Razorpay and confirm the plan supports customer checkout. Avoid stacking many unpaid `created` subscriptions for the same customer when testing plan switches—cancel stale attempts in the Dashboard if checkout behaves oddly.
